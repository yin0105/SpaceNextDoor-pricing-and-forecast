...
```{r}
lmodel.Train <- lm (price_cents ~ `area squared foot` + Locker + Unit + `Air con` + `24 hours` + `Lift access` + `Loading dock` + `Step ladder` + `Racking/Shelving`, data = Training)

summary(lmodel.Train)
```

```{r}
lmodel.Test <- lm (price_cents ~ `area squared foot` + Locker + Unit + `Air con` + `24 hours` + `Lift access` + `Loading dock` + `Step ladder` + `Racking/Shelving`, data = Test)
